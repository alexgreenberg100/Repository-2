...
def pie(site_dropdown):
    if site_dropdown == 'ALL':
        ...
        pie_fig=px.pie(filtered_df,values='class count',names='class',title=f"Success Launches for site: {site_dropdown}")
        return pie_fig
# TASK 4:
...
def scatter(site_dropdown,slider_range):
    ...
    if site_dropdown=='ALL':
        ...
        filtered_scatter=df_scatterf[df_scatter['Launch Site']==site_dropdown]
        scatter_fig=px.scatter(filtered_scatter,x='Payload Mass (kg)',y='class',color='Booster Version Category',title='Payload Success Rate for {site_dropdown}")
        return scatter_fig
